...
jQuery(document).ready(function($) {
    ...
    function blogigClickOutSideElm(elm, callback) {
    ...
    $( ".blogig-repeater-control" ).each(function() {
        var container = $(this)
        ...
        container.on( "click", ".fontawesome-icon-picker .icons-list i", function() {
            ...
            _this.parents( ".blogig-repeater-item" ).find( ".item-heading-wrap .item-heading" ).text(newValue.split("-")[1])
            repeater_value_refresh(_this);
        })

        // sortable handler
        container.find( ".blogig-repeater-control-inner" ).sortable({
            orientation: "vertical",
            items: "> .blogig-repeater-item",
            update: function (event, ui) {
                $(this).find( " > .blogig-repeater-item .remove-item" ).show()
                $(this).find( ".blogig-repeater-item:first .remove-item" ).hide()
        ...
        container.on( "click", ".display-icon", function() {
            ...
            _this.parents( ".blogig-repeater-item" ).toggleClass( "not-visible visible" )
            // _this.parent().next().toggleClass("isShow isHidden");
            // _this.parents( ".blogig-repeater-item" ).toggleClass("popupActive");
            repeater_value_refresh(_this)
        })
        ...
        container.on( "click", ".remove-item", function(e) {
            e.preventDefault();
            var controller = $(this).parents( ".blogig-repeater-control" ), toRemove = $(this).parents( ".blogig-repeater-item" );
        ...
        container.on( "click", ".add-new-item", function(e) {
            ...
            blogigClickOutSideElm( fieldsPopup, function() {
                fieldsPopup.parents( ".blogig-repeater-item" ).removeClass("popupActive")
        ...
        container.on( "click", ".blogig-repeater-item.visible .settings-icon", function() {
            $(this).parents(".blogig-repeater-item").siblings().removeClass("popupActive").find(".item-control-fields").removeClass("isShow").addClass("isHidden")
            $(this).parents(".blogig-repeater-item").siblings().find( ".fontawesome-icon-picker .icons-list" ).hide()
            $(this).parents(".blogig-repeater-item").toggleClass("popupActive")
            $(this).parent().next().toggleClass( "isHidden isShow" );
            $(this).parent().next().find( ".fontawesome-icon-picker .icons-list" ).hide();
            // close popup on outside click
            var fieldsPopup =  $(this).parents(".blogig-repeater-item").find( ".item-control-fields.isShow" )
            blogigClickOutSideElm( fieldsPopup, function() {
                fieldsPopup.parents( ".blogig-repeater-item" ).removeClass("popupActive")
                fieldsPopup.removeClass( "isShow" ).addClass( "isHidden" )
            })
        ...
        container.on( "change keyup", ".blogig-repeater-item .repeater-field-value-holder", function() {
            var _this = $(this)
            repeater_value_refresh(_this)
        })
    ...
    function repeater_value_refresh( _this ) {
        var controlValue = [], container =  ( _this.hasClass( "blogig-repeater-control" ) ) ? _this : _this.parents( ".blogig-repeater-control" );
        container.find( ".blogig-repeater-item" ).each(function() {
            var newValue = {}
